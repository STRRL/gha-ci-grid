...
export default async function handler(
  ...
  const workflowID = parseInt(req.query.workflow_id! as string);

  const ghClient = await rawClient.spawnInstallationClientForRepo(owner, repo);
  res.status(200).json(await ghClient.listGiHubActionRunsWithOwnerRepoWorklfowID(owner, repo, workflowID))
}